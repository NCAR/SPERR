...
#include "chngolombencode.h"

#define USG_STRING "[-s %:] [-vo %:] [-p %f:prob] [-m %d:m] %s:channel %s:bitstream"
...
int main(int argc, char *argv[])
{
  ...
  if (QccENTGolombEncodeChannel(&Channel,
                                &OutputBuffer,
                                (Prob == 0) ? NULL : &Prob,
                                (M == 0) ? NULL : &M))
    {
      QccErrorAddMessage("%s: Error calling QccENTGolombEncodeChannel()",
                         argv[0]);
      QccErrorExit();
    }
  ...
  rate = (double)(OutputBuffer.bit_cnt) / Channel.channel_length;
  ...
  if (!Silent)
    {
      if (ValueOnly)
      ...
      else
        {
          ...
          printf("using Golomb runlength coding.\n");
        }
    }

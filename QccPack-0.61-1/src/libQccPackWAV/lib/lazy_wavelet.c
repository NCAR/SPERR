                       ...
                       int subsample_pattern_col)
{
  ...
  if ((input_column = QccVectorAlloc(num_rows)) == NULL)
    {
      QccErrorAddMessage("(QccWAVWaveletLWT2D): Error calling QccVectorAlloc()");
      goto Error;
    }
  ...
  for (row = 0; row < num_rows; row++)
    if (QccWAVWaveletLWT(input_matrix[row],
                         output_matrix[row],
                         num_cols,
                         origin_row,
                         subsample_pattern_row))
      {
        QccErrorAddMessage("(QccWAVWaveletLWT2D): Error calling QccWAVWaveletLWT()");
        goto Error;
      }
                              ...
                              int subsample_pattern_col)
{
  ...
  if ((input_column = QccVectorAlloc(num_rows)) == NULL)
    {
      QccErrorAddMessage("(QccWAVWaveletInverseLWT2D): Error calling QccVectorAlloc()");
      goto Error;
    }
  if ((output_column = QccVectorAlloc(num_rows)) == NULL)
    {
  ...
  for (row = 0; row < num_rows; row++)
    if (QccWAVWaveletInverseLWT(input_matrix[row],
                                output_matrix[row],
                                num_cols,
                                origin_row,
                                subsample_pattern_row))
      {
        QccErrorAddMessage("(QccWAVWaveletInverseLWT2D): Error calling QccWAVWaveletInverseLWT()");
        goto Error;
      }
                       ...
                       int subsample_pattern_col)
{
  ...
  if ((input_vector = QccVectorAlloc(num_frames)) == NULL)
    {
      QccErrorAddMessage("(QccWAVWaveletLWT3D): Error calling QccVectorAlloc()");
      goto Error;
    }
  if ((output_vector = QccVectorAlloc(num_frames)) == NULL)
    {
      QccErrorAddMessage("(QccWAVWaveletLWT3D): Error calling QccVectorAlloc()");
      goto Error;
    }
  ...
  for (frame = 0; frame < num_frames; frame++)
    if (QccWAVWaveletLWT2D(input_volume[frame],
                           output_volume[frame],
                           num_rows,
                           num_cols,
                           origin_row,
                           ...
                           subsample_pattern_col))
      {
        QccErrorAddMessage("(QccWAVWaveletLWT3D): Error calling QccWAVWaveletLWT2D()");
        goto Error;
      }
    ...
    for (col = 0; col < num_cols; col++)
      {
        ...
        if (QccWAVWaveletLWT(input_vector,
                             output_vector,
                             num_frames,
                             origin_frame,
                             subsample_pattern_frame))
          {
            QccErrorAddMessage("(QccWAVWaveletLWT3D): Error calling QccWAVWaveletLWT()");
            goto Error;
          }
                              ...
                              int subsample_pattern_col)
{
  ...
  if ((input_vector = QccVectorAlloc(num_frames)) == NULL)
    {
      QccErrorAddMessage("(QccWAVWaveletInverseLWT3D): Error calling QccVectorAlloc()");
      goto Error;
    }
  if ((output_vector = QccVectorAlloc(num_frames)) == NULL)
    {
  ...
  for (frame = 0; frame < num_frames; frame++)
    if (QccWAVWaveletInverseLWT2D(input_volume[frame],
                                  output_volume[frame],
                                  num_rows,
                                  num_cols,
                                  origin_row,
                                  ...
                                  subsample_pattern_col))
      {
        QccErrorAddMessage("(QccWAVWaveletInverseLWT3D): Error calling QccWAVWaveletInverseLWT2D()");
        goto Error;
      }
    ...
    for (col = 0; col < num_cols; col++)
      {
        ...
        if (QccWAVWaveletInverseLWT(input_vector,
                                    output_vector,
                                    num_frames,
                                    origin_frame,
                                    subsample_pattern_frame))
          {
            QccErrorAddMessage("(QccWAVWaveletInverseLWT3D): Error calling QccWAVWaveletInverseLWT()");
            goto Error;
          }

...
int main(int argc, char *argv[])
{
  ...
  if (QccWAVwdrDecodeHeader(&InputBuffer,
                            &NumLevels,
                            &NumRows,
                            &NumCols,
                            &ImageMean,
                            &MaxCoefficientBits))
    {
      QccErrorAddMessage("%s: Error calling QccWAVwdrDecodeHeader()", argv[0]);
      QccErrorExit();
    }
  ...
  if (MaskSpecified)
    {
      if (QccIMGImageRead(&Mask))
        {
          QccErrorAddMessage("%s: Error calling QccIMGImageRead()", argv[0]);
          QccErrorExit();
        }
      if (QccIMGImageColor(&Mask))
        {
          QccErrorAddMessage("%s: Mask must be grayscale", argv[0]);
          QccErrorExit();
        }
      if (QccIMGImageGetSize(&Mask, &MaskNumRows, &MaskNumCols))
        {
      ...
      if ((MaskNumRows != NumRows) || (MaskNumCols != NumCols))
        {
          QccErrorAddMessage("%s: Mask must be same size as image", argv[0]);
          QccErrorExit();
        }
  ...
  if (QccIMGImageSetSize(&OutputImage, NumRows, NumCols))
    {
  ...
  if (QccIMGImageAlloc(&OutputImage))
    {
      QccErrorAddMessage("%s: Error calling QccIMGImageAlloc()", argv[0]);
      QccErrorExit();
    }
      ...
      if (PerceptualWeights.num_subbands != NumSubbands)
        {
          QccErrorAddMessage("%s: Number of subbands in %s does not match that specified in %s",
                             argv[0],
                             PerceptualWeights.filename, InputBuffer.filename);
        }
    }
  ...
  if (QccWAVwdrDecode(&InputBuffer,
                      &(OutputImage.Y),
                      ...
                      TargetBitCnt))
    {
      QccErrorAddMessage("%s: Error calling QccWAVwdrDecode()", argv[0]);
      QccErrorExit();
    }
  
  if (QccIMGImageWrite(&OutputImage))
    {
      QccErrorAddMessage("%s: Error calling QccIMGImageWrite()", argv[0]);
      QccErrorExit();
    }

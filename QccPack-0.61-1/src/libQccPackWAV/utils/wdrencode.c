...
#define USG_STRING "[-w %s:wavelet] [-b %s:boundary] [-pw %: %s:pcpfile] [-nl %d:num_levels] [-m %: %s:mask] [-vo %:] %f:rate %s:imgfile %s:bitstream"
...
int main(int argc, char *argv[])
{
      ...
      if (PerceptualWeights.num_subbands != NumSubbands)
        {
          QccErrorAddMessage("%s: Number of subbands in %s does not match specified number of levels (%d)",
                             argv[0], PerceptualWeights.filename, NumLevels);
        }
    }
  ...
  if (QccIMGImageColor(&InputImage))
    {
      QccErrorAddMessage("%s: Color images not supported", argv[0]);
      QccErrorExit();
    }
  if (QccIMGImageGetSize(&InputImage, &ImageNumRows, &ImageNumCols))
    {
  ...
  if (MaskSpecified)
    {
      ...
      if ((MaskNumRows != ImageNumRows) || (MaskNumCols != ImageNumCols))
        {
  ...
  if (QccWAVwdrEncode(&(InputImage.Y),
                      ...
                      TargetBitCnt))
    {
      QccErrorAddMessage("%s: Error calling QccWAVwdrEncode()", argv[0]);
      QccErrorExit();
    }

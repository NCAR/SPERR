...
#define USG_STRING "[-w %s:multiwavelet] [-nl %d:num_levels] [-z %:] %s:infile %s:outfile"
...
int main(int argc, char *argv[])
{
  ...
  if (QccWAVMultiwaveletDMWT1D(&InputDataset,
                               &OutputDataset,
                               NumLevels,
                               &Multiwavelet))
    {
      QccErrorAddMessage("%s: Error calling QccWAVMultiwaveletDMWT1D()",
                         argv[0]);
      QccErrorExit();
    }

  if (ZeroHighPassBands)
    {
      baseband_length =
        QccWAVWaveletDWTSubbandLength(InputDataset.num_vectors,
                                      NumLevels, 0, 0, 0);
      for (index = baseband_length;
           index < OutputDataset.num_vectors;
           index++)
        QccVectorZero(OutputDataset.vectors[index],
                      OutputDataset.vector_dimension);
    }

...
#include "klttcedecode3d_lossless.h"
...
int main(int argc, char *argv[])
{
  ...
  if (QccWAVklttce3DLosslessDecodeHeader(&InputBuffer,
                                         &NumLevels,
                                         &NumFrames,
                                         &NumRows,
                                         &NumCols,
                                         ...
                                         &Alpha))
    {
      QccErrorAddMessage("%s: Error calling QccWAVklttce3DLosslessDecodeHeader()",
                         argv[0]);
      QccErrorExit();
    }
  ...
  if (QccWAVklttce3DLosslessDecode(&InputBuffer,
                                   &OutputImage,
                                   NumLevels,
                                   Alpha,
                                   &Wavelet,
                                   ...
                                   TargetBitCnt))
    {
      QccErrorAddMessage("%s: Error calling QccWAVklttce3DLosslessDecode()",
                         argv[0]);
      QccErrorExit();
    }

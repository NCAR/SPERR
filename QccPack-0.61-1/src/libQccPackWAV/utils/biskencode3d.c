...
#include "biskencode3d.h"

#define USG_STRING "[-w %s:wavelet] [-b %s:boundary] [-nl %: %d:num_levels] [-sl %: %d:spatial_num_levels] [-tl %: %d:temporal_num_levels] [-m %: %s:mask] [-vo %:] %f:rate %s:icbfile %s:bitstream"
...
int main(int argc, char *argv[])
{
  ...
  if (QccIMGImageCubeRead(&InputImage))
    {
      QccErrorAddMessage("%s: Error calling QccIMGImageCubeReadData",
                         argv[0]);
      QccErrorExit();
    }
  ...
  if (QccWAVbisk3DEncode(&InputImage,
			 ...
			 SpatialNumLevels,
                         ...
                         TargetBitCnt))
    {
      QccErrorAddMessage("%s: Error calling QccWAVbiskEncode3D()",
                         argv[0]);
      QccErrorExit();
    }

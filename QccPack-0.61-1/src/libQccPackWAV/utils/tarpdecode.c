...
int main(int argc, char *argv[])
{
  ...
  if (QccWAVTarpDecodeHeader(&InputBuffer,
                             &Alpha,
                             &NumLevels,
                             &NumRows, &NumCols,
                             &ImageMean,
                             &MaxCoefficientBits))
    {
      QccErrorAddMessage("%s: Error calling QccWAVTarpDecodeHeader()",
                         argv[0]);
      QccErrorExit();
    }
  ...
  if (QccWAVTarpDecode(&InputBuffer,
                       &(OutputImage.Y),
                       ...
                       TargetBitCnt))
    {
      QccErrorAddMessage("%s: Error calling QccWAVTarpDecode()",
                         argv[0]);
      QccErrorExit();
    }

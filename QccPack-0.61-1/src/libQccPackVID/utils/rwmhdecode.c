...
int main(int argc, char *argv[])
{
  ...
  if (QccVIDRWMHDecodeHeader(&InputBuffer,
                             &NumRows,
                             &NumCols,
                             ...
                             &TargetBitCnt))
    {
      QccErrorAddMessage("%s: Error calling QccVIDRWMHDecodeHeader()",
                         argv[0]);
      QccErrorExit();
    }
  ...
  if (QccVIDRWMHDecode(&ImageSequence,
                       ((FilterSpecified1) ?
                        &Filter1 : NULL),
                       ((FilterSpecified2) ?
                        &Filter2 : NULL),
                       ...
                       Quiet))
    {
      QccErrorAddMessage("%s: Error calling QccVIDRWMHDecode()",
			 argv[0]);
      QccErrorExit();
    }

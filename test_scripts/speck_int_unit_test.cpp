    ...
    -> std::pair<std::vector<T>, sperr::Bitmask>
{
  ...
  auto signs = sperr::Bitmask(len * 2);
  signs.reset_true();
...
TEST(SPECK1D_INT, Random1)
{
  ...
  for (size_t i = 0; i < input.size(); i++)
    if (input[i])
      EXPECT_EQ(input_signs.rbit(i * 2 + 1), output_signs.rbit(i * 2 + 1));
}

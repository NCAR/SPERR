 ...
 public:
  ...
  void gen_outliers(size_t N)
  {
    ...
    while (LOS.size() < N) {
      ...
      auto loc = loc_d(gen);
      while (
          std::any_of(LOS.begin(), LOS.end(), [loc](auto& out) { return out.location == loc; })) {
        loc = loc_d(gen);
      }
  ...
  {
    for (auto& orig : LOS) {
      if (!std::any_of(recovered.cbegin(), recovered.cend(), [&orig, this](auto& recov) {
            return (recov.location == orig.location &&

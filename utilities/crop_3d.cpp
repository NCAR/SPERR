...
int main(int argc, char* argv[]) {
  if (argc != 12) {
    std::cerr
        << "Usage: InFileName, InFileNX, InFileNY, InFileNZ, "
        ...
        << "to index 128 (exclusive) in X dimension, just type 64 and 128!"
        << std::endl;
    exit(1);
  }
   ...
   */
  auto inDims = std::array<size_t, 3>{std::atol(argv[2]), std::atol(argv[3]),
                                      std::atol(argv[4])};
  ...
  auto outDims = std::array<size_t, 3>{
      outFinishX - outStartX, outFinishY - outStartY, outFinishZ - outStartZ};

...
int main(int argc, char* argv[]) {
    ...
    //
    // It is the user's responsibility to make sure the range and dimensions are good.
    //
    auto big_dims   = std::array<size_t, 3>{std::atol(argv[2]), std::atol(argv[3]), std::atol(argv[4])};
    auto small_dims = std::array<size_t, 3>{std::atol(argv[6]), std::atol(argv[7]), std::atol(argv[8])};

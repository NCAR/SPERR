...
int main( int argc, char** argv )
{
    ...
    if( num_of_rows < min_xyz )     min_xyz = (float)num_of_rows;
    if( num_of_frms < min_xyz )     min_xyz = (float)num_of_frms;
    float tmpf = log2f( min_xyz / 8.0f );
    int num_levels = tmpf < 0.0f ? 0 : (int)tmpf + 1;

	/* forward transform */
	QccWAVSubbandPyramid3DDWT( &coeff3d, QCCWAVSUBBANDPYRAMID3D_PACKET, num_levels, num_levels, &cdf97 );

	/* inverse transform */
	QccWAVSubbandPyramid3DInverseDWT( &coeff3d, &cdf97 );

    /* copy result to an output buffer */
    double* out_buf = (double*) malloc( num_of_vals * sizeof(double) );
    counter = 0;
	for( size_t frame = 0; frame < num_of_frms; frame++ )
		for( size_t row = 0; row < num_of_rows; row++ )
			for( size_t col = 0; col < num_of_cols; col++ )
				buf[ counter++ ] = coeff3d.volume[frame][row][col];

    // write result to qccpack_result.raw
    f = fopen( output_name, "w" );
    fwrite( buf, sizeof(double), num_of_vals, f );
    fclose( f );

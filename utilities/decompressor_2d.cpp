...
int main( int argc, char* argv[] )
{
    ...
    size_t dim_x, dim_y;
    decoder.get_dims( dim_x, dim_y );
    size_t total_vals = dim_x * dim_y;
    ...
    idwt.take_data( decoder.release_coeffs_double(), total_vals );
    ...
    std::printf("# Decompressed volume with dimensions (%lu, %lu). Actual BPP = %f\n", 
                 dim_x, dim_y, float(bit_budget)/float(total_vals)  );

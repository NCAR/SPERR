...
auto SPERR2D_Compressor::set_target_psnr(double psnr) -> RTNType
{
  if (psnr <= 0.0)
    return RTNType::InvalidParam;
...
//#endif
  ...
  auto speck_bit_budget = m_bit_budget - (m_meta_size + m_condi_stream.size()) * 8;
...
auto SPERR2D_Compressor::m_assemble_encoded_bitstream() -> RTNType
{
  ...
  // bool_byte[2]  : if there is SPERR stream (true) or not (false).

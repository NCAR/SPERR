...
auto sperr::SPECK2D_INT_ENC<T>::m_decide_S_significance(const Set2D& set) const -> bool
{
  ...
  for (auto y = set.start_y; y < (set.start_y + set.length_y); y++) {
    ...
    bool sig = false;
#ifdef __AVX2__
    sig = sperr::any_ge_pow2(first, set.length_x, m_threshold);
#else
    sig = std::any_of(first, first + set.length_x,
...
auto sperr::SPECK2D_INT_ENC<T>::m_decide_I_significance() const -> bool
{
  ...
  for (auto y = 0u; y < m_I.start_y; y++) {
    ...
    bool sig = false;
#ifdef __AVX2__
    sig = sperr::any_ge_pow2(first, len, m_threshold);
#else
    sig = std::any_of(first, first + len, [thld = m_threshold](auto v) { return v >= thld; });
#endif
    if (sig)
      return true;
  }

...
// Note 2: This class only works on little endian machines (99.9% of all machines).
//
...
private:
    ...
    // When m_cache_vec_pos < m_vec.size(), m_cache keeps a byte from m_vec unpacked.
    // When m_cache_vec_pos == m_vec.size(), m_cache keeps 8 booleans to be packed.
    // When m_cache_vec_pos > m_vec.size(), something is wrong.

...
void sperr::SPECK3D_FLT::m_instantiate_encoder()
{
  switch (m_uint_flag) {
    case UINTType::UINT64:
      if (m_encoder.index() != 0 || std::get_if<0>(&m_encoder) == nullptr)
...
void sperr::SPECK3D_FLT::m_instantiate_decoder()
{
  switch (m_uint_flag) {
    case UINTType::UINT64:
      if (m_decoder.index() != 0 || std::get_if<0>(&m_decoder) == nullptr)
        m_decoder = std::make_unique<SPECK3D_INT_DEC<uint64_t>>();
      break;
    case UINTType::UINT32:
      if (m_decoder.index() != 1 || std::get_if<1>(&m_decoder) == nullptr)
        m_decoder = std::make_unique<SPECK3D_INT_DEC<uint32_t>>();
      break;
    case UINTType::UINT16:
      if (m_decoder.index() != 2 || std::get_if<2>(&m_decoder) == nullptr)
        m_decoder = std::make_unique<SPECK3D_INT_DEC<uint16_t>>();
      break;
    default:
      if (m_decoder.index() != 3 || std::get_if<3>(&m_decoder) == nullptr)

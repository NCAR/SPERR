...
auto sperr::SPECK3D_INT::m_partition_S_XYZ(const Set3D& set) const -> std::array<Set3D, 8>
{
...
auto sperr::SPECK3D_INT::m_partition_S_XY(const Set3D& set) const -> std::array<Set3D, 4>
{
...
auto sperr::SPECK3D_INT::m_partition_S_Z(const Set3D& set) const -> std::array<Set3D, 2>
{
...
auto sperr::SPECK3D_INT::m_decide_significance(const Set3D& set) const
    -> std::pair<SigType, std::array<uint32_t, 3>>
{
  ...
  for (auto z = set.start_z; z < (set.start_z + set.length_z); z++) {
    ...
    for (auto y = set.start_y; y < (set.start_y + set.length_y); y++) {
      ...
      if (found != last) {
        return {SigType::Sig, {std::distance(first, found), y - set.start_y, z - set.start_z}};
      }
    }
  }

...
/* extern_ const size_t stream_word_bits; */ /* bit stream granularity */ /* Sam removal */
...
uint test_range(bitstream* s, bitstream_offset start_pos, bitstream_offset range_len);

/* Sam addition: read a bit at a random position. */
uint random_read_bit(const bitstream* s, bitstream_offset pos);

/* Sam addition: write a bit at a random position and flush (in a safe manner). */
uint random_write_bit(bitstream* s, uint bit, bitstream_offset pos);
